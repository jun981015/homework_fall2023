...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward + self.discount*next_qs*torch.logical_not(done)
        ...
        # Compute loss
        loss: torch.Tensor = self.critic_loss(q_values.mean(dim=0),target_values.mean(dim=0))
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return -action_distribution.log_prob(action).mean()
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs.reshape(self.num_actor_samples,batch_size,-1),action)
